...
describe(PLUGIN_NAME .. ": schema", function()
  ...
  it("applies the new caching/forwarding defaults", function()
    local ok = validate({ url = "http://middle.test" })
    assert.same({}, ok.config.cache_response_codes)
    assert.equal(0, ok.config.cache_storage_ttl)
    assert.is_false(ok.config.cache_control)
    assert.same({}, ok.config.forward_headers_allow)
    assert.same({}, ok.config.forward_response_headers)
  end)

  it("rejects an out-of-range cache_response_codes entry", function()
    local ok, err = validate({ url = "http://middle.test", cache_response_codes = { 700 } })
    assert.is_falsy(ok)
    assert.is_not_nil(err.config.cache_response_codes)
  end)
end)
