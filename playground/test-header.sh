...
# Per-run tokens so each run starts from a COLD cache key (cache_based_on=header),
# making the suite re-runnable within the cache_ttl window.
...
R=$(curl -s -H "Authorization: $TOK1" "$URL")
...
R=$(curl -s -H "Authorization: $TOK2" "$URL")
